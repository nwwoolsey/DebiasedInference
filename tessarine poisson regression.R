...
pois<-function(data){
  ...
  score<-function(theta){
    b0<-theta[1]
    b1<-theta[2]
    score1<-mean(y-Re(exp(b0+b1*(w+b*1i))*cosh(b1*(c+d*1i))))^2
    score2<-mean(y*w-Re(exp(b0+b1*(w+b*1i))*((w+b*1i)*cosh(b1*(c+d*1i))+(c+d*1i)*sinh(b1*(c+d*1i)))))^2
    return(score1+score2)
  }
  nai<-naivepois(data)
  out<-optim(nai,fn=score,method="BFGS")$par
...
comp<-function(data){
  ...
  score<-function(theta){
    b0<-theta[1]
    b1<-theta[2]
    score1<-sum(Re(y-exp(b0+b1*w+b1*1i*sqrt(Vastar))))^2
    score2<-sum(Re(y*w-(w+1i*sqrt(Vastar))*exp(b0+b1*w+1i*b1*sqrt(Vastar))))^2
    return(score1+score2)
  }
  nai<-naivepois(data)
  out<-optim(c(0,0),fn=score,method="BFGS")$par
...
cs<-function(data){
  ...
  score<-function(theta){
    b0<-theta[1]
    b1<-theta[2]
    score0<-mean(y-exp(b0-b1^2/2*Vastar+b1*w))
    score1<-mean(w*y-exp(b0-Vastar*b1^2/2+b1*w)*(w-Vastar*b1))
    return(score0^2+score1^2)
  }
  nai<-naivepois(data)
  out<-optim(nai,fn=score,method="BFGS")$par
...
b0<-1
b1<--1
x<-matrix(rnorm(n*samp,0,1),nrow=n,ncol=samp)
lambda<-exp(b0+b1*x)
y<-apply(lambda,2,rpois,n=n)
N<-1
B<-2/N
#B<-2
mom<-c()
...
for(delta in deltas){
  ...
  uni<-matrix(runif(samp*3,1-delta,1+delta),nrow=3,ncol=samp)
  w<-x+u
  MU<-A/B
  Va<-A/B^2
  Skew<-2*A/B^3
  Kurt<-3*A*(2+A)/B^4
  data<-rbind(w,y,uni)
  new<-apply(data,2,pois)
  stef<-apply(data,2,cs)
  nai<-apply(data,2,naivepois)
  true<-apply(rbind(x+MU,y),2,naivepois)
  truebias1<-append(truebias1,(mean(true[2,]-b1)))
  truemse1<-append(truemse1,mean((true[2,]-b1)^2))
  newbias1<-append(newbias1,(mean(new[2,]-b1)))
  newmse1<-append(newmse1,mean((new[2,]-b1)^2))
...
par(mfrow=c(2,3))
par(mar=c(4,4,3,1))
plot(NULL,xlim=c(0,.6),ylim=c(-.04,max(naivebias0)),xlab=TeX(r'($\delta$)'),ylab="",main=TeX(r'(Bias of $\hat{\beta}_{0}$)'),las=1)
lines(deltas,naivebias0,lty=2,col="blue",lwd=1.5)
lines(deltas,truebias0,lty=3,col="orange",lwd=1.5)
lines(deltas,stefbias0,lty=4,col="chartreuse4",lwd=1.5)
lines(deltas,newbias0,lty=1,col="red",lwd=1.5)




plot(NULL,xlim=c(0,.6),ylim=c(min(truemse0),.006),xlab=TeX(r'($\delta$)'),ylab="",main=TeX(r'(Mean Squared Error of $\hat{\beta}_{0}$)'),las=1)
lines(deltas,naivemse0,lty=2,col="blue",lwd=1.5)
lines(deltas,newmse0,lty=1,col="red",lwd=1.5)
lines(deltas,truemse0,lty=3,col="orange",lwd=1.5)
lines(deltas,stefmse0,lty=4,col="chartreuse4",lwd=1.5)
...
plot(NULL,xlim=c(0,.6),ylim=c(.055,max(c(newiqr0,stefiqr0,naiveiqr0))),xlab=TeX(r'($\delta$)'),ylab="", main=TeX(r'(IQR of $\hat{\beta}_{0}$)'),las=1)
lines(deltas,naiveiqr0,lty=2,col="blue",lwd=1.5)
lines(deltas,trueiqr0,lty=3,col="orange",lwd=1.5)
lines(deltas,stefiqr0,lty=4,col="chartreuse4",lwd=1.5)
lines(deltas,newiqr0,lty=1,col="red",lwd=1.5)

plot(NULL,xlim=c(0,.6),ylim=c(-.06,max(naivebias1)),xlab=TeX(r'($\delta$)'),ylab="",main=TeX(r'(Bias of $\hat{\beta}_{1}$)'),las=1)
lines(deltas,naivebias1,lty=2,col="blue",lwd=1.5)
lines(deltas,truebias1,lty=3,col="orange",lwd=1.5)
lines(deltas,stefbias1,lty=4,col="chartreuse4",lwd=1.5)
lines(deltas,newbias1,lty=1,col="red",lwd=1.5)

plot(NULL,xlim=c(0,.6),ylim=c(min(truemse1),max(stefmse1)),xlab=TeX(r'($\delta$)'),ylab="",main=TeX(r'(Mean Squared Error of $\hat{\beta}_{1}$)'),las=1)
lines(deltas,naivemse1,lty=2,col="blue",lwd=1.5)
lines(deltas,newmse1,lty=1,col="red",lwd=1.5)
lines(deltas,truemse1,lty=3,col="orange",lwd=1.5)
lines(deltas,stefmse1,lty=4,col="chartreuse4",lwd=1.5)

plot(NULL,xlim=c(0,.6),ylim=c(.04,max(c(newiqr1,stefiqr1,naiveiqr1))),xlab=TeX(r'($\delta$)'),ylab="", main=TeX(r'(IQR of $\hat{\beta}_{1}$)'),las=1)
lines(deltas,naiveiqr1,lty=2,col="blue",lwd=1.5)
lines(deltas,trueiqr1,lty=3,col="orange",lwd=1.5)
lines(deltas,stefiqr1,lty=4,col="chartreuse4",lwd=1.5)
lines(deltas,newiqr1,lty=1,col="red",lwd=1.5)
